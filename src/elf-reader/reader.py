...
def partition_lib_symbols(lib_path, symbols, filter_fnc=lambda x: False):
    ...
    for symbol in symbols:
        if symbol["path"].startswith(lib_path) \
                and symbol["type"] != "t" and symbol["type"] != "T" and not filter_fnc(symbol):
...
def create_first_symbol_group(symbols):
    ...
    """
    group = {"min_address": symbols[0]["address"], "max_address": symbols[0]["address"] + symbols[0]["size"], "symbols": [symbols[0]]}
...
def split_groups_with_other_symbols(groups, other_symbols):
    for other_symbol in other_symbols:
        ...
        while i < len(groups):
            group = groups[i]
            if (group["min_address"] < other_symbol["address"] < group["max_address"]) or \
                (group["min_address"] < other_symbol["address"] + other_symbol["size"] < group["max_address"]):
                new_group = {"min_address": group["max_address"], "max_address": group["min_address"], "symbols": []}
                j = 0
                while j < len(group["symbols"]):
                    group_symbol = group["symbols"][j]
                    if other_symbol["address"] >= group_symbol["address"] + group_symbol["size"]:
                        j += 1
                    else:
                ...
                group["max_address"] = max([symbol["address"] + symbol["size"] for symbol in group["symbols"]])
            i += 1
...
def main(elf_file, wamr_path, external_env_var_name, external_env_count_var_name):
    ...
    filter_function = lambda x: x["type"] in ignored_symbols_types or x["symbol"] in ignored_symbols
    lib_symbols, other_symbols = partition_lib_symbols(wamr_path, symbols, filter_function)
